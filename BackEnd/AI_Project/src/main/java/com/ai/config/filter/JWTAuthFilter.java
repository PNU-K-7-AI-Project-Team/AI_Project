...
public class JWTAuthFilter extends UsernamePasswordAuthenticationFilter{
	...
	// AuthenticationException: 인증 예외 발생 처리
	public Authentication attemptAuthentication(HttpServletRequest request,
		...
		try { // 요청 데이터를 JSON으로 읽어 Mapper(Java 객체)로 변환하고 user 객체에 저장
			...
			// authenticationManager.authenticate: 인증 시도 후 자격증명의 정보를 검증한뒤 인증 성공 시 토큰 반환  
		} catch (Exception e) {
	...
	// FilterChain: 요청들을 다음 필터들로 전달하는 용도의 객체
		...
		String token = JWT.create() // 새로운 JWT 토큰 생성
				...
				.withClaim("userId", user.getUserId()) // 클레임에 유저 아이디 추가
				.sign(Algorithm.HMAC256("sign")); // 서명
